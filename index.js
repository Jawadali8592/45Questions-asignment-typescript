...
let message = "Khudi ko kar buland itna, ke har taqdeer se pehle, Khuda bande se khud pooche, 'Bata, teri raza kya hai?";
...
let personNameWithWhiteSpace = "\t QUAID E AZAM \n";
...
var favNumber = 786;
var messageFav = "MY favourate number is =";
...
let myFriends = ["maan", "Hassan", "Ubaid", "Muzammil", "Abdullah"];
...
let messageToFriends = "Hope you have a great day";
...
let bike = "HONDA YBR";
let motorcycle = ["Commuting: Bicycles are an eco-friendly and efficient", "Exercise and Fitness: Many people use bikes for exercise and fitness,", "Touring: Bicycle touring involves long-distance travel by bike,", "bike is very useful to travel"];
...
let guests = ["MIAN HASSAN", "ABDULLAH JUTT", "FATIMA S"];
...
let places = ["MAKKAH", "MADINA", "switzrland", "GERMANY", "BRAZIL"];
...
let thinkLanguage = ["PUNJABI", "SINDHI", "BALOCHI", "PASHTO", "URDU"];
...
if (favorite_fruits.includes('ORANGE')) {
    ...
    console.log("ORANGE is not in THREE FRIUTS LIST");
}
...
let userNames = ["Admin", "Watson", "Maxwell", "Williams", "John"];
...
let current_users = ["Ahmed", "Ali", "Usama", "Basit", "Ahmar"];
// Make another list of five usernames called new_users
let new_users = ["Riaz", "Tanveer", "Ali", "Noman", "Usama"];
for (let index = 0; index < new_users.length; index++) {
    if (current_users.includes(new_users[index])) {
        ...
        console.log(new_users[index], "CONGRATS,Ther user name is available");
    }
}
...
//
let favouratePizza = ["MALAI BOTI", "CHICKEN FAJITA", "KABAB PIZZA"];
...
function makeshirt(shirts) {
    if (shirts == "Large") {
        console.log("I love Typescript");
    }
    else if (shirts == "Medium") {
...
let magicianName = ["Alex", "John", "Alice", "King", "Bruce"];
...
function show_magician(megician) {
    const nwMagician = magicianName.map(() => `the great ${magicianName}`);
    return nwMagician;
}
...
function carFun(manufacturer, Model, color, extrafeature) {
    let carS = {
        manufacturer,
        Model,
        color,
